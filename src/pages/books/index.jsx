  ...
  return (
    <div className='books-page page'>
     ...
     <div className="btn-group w-75 mx-auto  flex justify-around mt-3 flex-wrap">
        ...
        {!loadBooks && (
          <div className="mt-4">
            <h2 className="text-center font-monospace fs-5">{t('countBooks')}: {books?.length}</h2>
            <div className="books_wrp flex items-center flex-wrap justify-around">
                {books?.map((el, idx)=>(
                    <BookCard isDelete={true} catId={categories[activeCat]?._id} reloadFunc={getBooks} key={idx} book={el} author={el.author}/>
                ))}
            </div>
          </div>
        )}
