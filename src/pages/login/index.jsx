...
const Login = () => {
  ...
  const loginHandler = async (e) => {
    ...
    try {
        setLoadLogin(true)
        const {data} = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/users/login-admin`, {
          email : emailRef?.current.value.trim(),
          password : passwordRef.current.value.trim()  
        })
        if(data?.success){
        toast.success(data?.message[currentLang])
        sessionStorage.setItem('userDatas', JSON.stringify({...data?.data, token : data?.token}))
        navigate('/')
        location.reload()
        }else{
